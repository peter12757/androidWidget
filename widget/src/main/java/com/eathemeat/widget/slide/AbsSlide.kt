package com.eathemeat.widget.slide
...
import com.eathemeat.widget.R
import com.eathemeat.widget.slide.anim.ThumbValueAnimation
import com.eathemeat.widget.slide.anim.TipViewAnimator
import com.eathemeat.widget.util.Utils
...
abstract class AbsSlider constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
    ...
    var trackHeight = 0
        set(@androidx.annotation.IntRange(from = 0) value) {
            if (value != field) {
                field = value
                updateViewLayout()
            }
    ...
    private fun drawThumb(canvas: Canvas, width: Int, yCenter: Float) {
        ...
        if (!thumbAnimation.isThumbHidden()) {
            ...
            val thumbDrawable = customThumbDrawable ?: ThumbDrawable
    ...
    /**
     * Returns a number between 0 and 1 with [AbsSlider.value]
    ...
    /**
     * 校验[AbsSlider.value]合法性，对不合法数据进行修正
     */
    ...
    fun viewHeightChanged(): Boolean {
        ...
        val thumbHeight = customThumbDrawable?.bounds?.height()?:ThumbDrawable.bounds.height()
    ...
    /**
     * Sets the slider's [AbsSlider.value]
    ...
    /**
     * Sets the slider's [AbsSlider.secondaryValue]
     *
     ...
     * 也可以使用 [AbsSlider.setThumbWithinTrackBounds] 来将thumb直接控制在track内部
     *
     ...
     * @param radius 圆角半径
     */
    fun setTrackCornersRadius(@androidx.annotation.IntRange(from = 0) @Dimension radius: Int) {
    ...
    var thumbRadius = 0
        set(@IntRange(from = 0) @Dimension radius) {
            ...
            field = radius
            ThumbDrawable.cornerSize = radius.toFloat()
    ...
    fun setThumbTintList(thumbColor: ColorStateList) {
        if (thumbColor == ThumbDrawable.fillColor) {
            return
        }
        ThumbDrawable.fillColor = thumbColor
        invalidate()
    }
     ...
     */
    fun setHaloRadius(@androidx.annotation.IntRange(from = 0) @Dimension radius: Int) {
    ...
    fun setThumbElevation(elevation: Float) {
        ...
        ThumbDrawable.elevation = elevation
    ...
    fun setThumbStrokeColor(thumbStrokeColor: ColorStateList?) {
        ThumbDrawable.strokeColor = thumbStrokeColor
        postInvalidate()
    }
    ...
    fun setThumbStrokeWidth(thumbStrokeWidth: Float) {
        ThumbDrawable.strokeWidth = thumbStrokeWidth
        postInvalidate()
    }
    ...
    fun setThumbShadowColor(@ColorInt shadowColor: Int) {
        ThumbDrawable.shadowColor = shadowColor
    }
     ...
     *  - [AbsSlider.MODE_NORMAL]
     *  - [AbsSlider.MODE_DISABLE_TOUCH]
     *  - [AbsSlider.MODE_DISABLE_CLICK_TOUCH]
     *
